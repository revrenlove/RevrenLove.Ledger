...
public abstract class LedgerDbContext(DbContextOptions options)
    : IdentityDbContext<LedgerUser, IdentityRole<Guid>, Guid>(options)
{
    ...
    /// <summary>
    /// Applies provider-specific configurations to the given model builder instance.
    /// </summary>
    /// <remarks>
    /// <para><b>Each provider needs to specify it's own check constraint for minimum length of a Description for FinancialTransactions.</b></para>
    /// Override this method in a derived class to customize model configuration for a specific
    /// database provider. This method is typically called during model creation to apply settings or conventions
    /// required by the provider.</remarks>
    /// <param name="modelBuilder">The model builder to which provider-specific configurations will be applied. Cannot be null.</param>
    /// <returns>A ModelBuilder instance with provider-specific configurations applied. The returned instance may be the same as
    /// the input or a modified version.</returns>
    protected abstract ModelBuilder ApplyProviderSpecificConfigurations(ModelBuilder modelBuilder);
